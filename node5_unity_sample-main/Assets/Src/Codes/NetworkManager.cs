...
public class NetworkManager : MonoBehaviour
{
    ...
    public void SendPingPacket()
    {
        ...
        Debug.Log(pingPacket);

        SendPacket(pingPacket, (uint)Packets.HandlerIds.Ping);
    }
    ...
    void HandlerPingPacket(byte[] packetdata)
    {
        ...
        {
            response = Packets.Deserialize<Ping>(packetdata);

			GameManager.instance.latency = (int)response.latency;
	...
	void HandleNormalPacket(byte[] packetData) {
        ...
        if (response.data != null && response.data.Length > 0) {
			...
			if (response.handlerId == 0) {
				payload = Packets.Deserialize<InitialPacket>(response.data);
				GameManager.instance.GameStart(payload);
            }
